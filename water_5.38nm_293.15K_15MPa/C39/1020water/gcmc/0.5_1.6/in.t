...
read_data          C39_H2_1020watergcmc0.5ns.data
...
variable 	      type18 atom "type==18"
group 	            type18 dynamic ggcmc var type18
variable          nH2 equal count(type18)
group             bH2 dynamic ggcmc region subbulk every 100
variable          nbH2 equal count(bH2)

compute           mtem type18 temp/com
...
fix               2 ggcmc gcmc 100 100 100 18 29494  ${T} 1 0.5 region soall pressure ${Patm} fugacity_coeff ${fe} group ggcmc

fix               mynvt h2o nvt temp ${T} ${T} 100
fix_modify        mynvt temp mwtem

fix               wshake h2o shake 0.0001 50 0 b 1 a 1 mol h2omol
fix               3 type18 nvt temp ${T}  ${T}  100 
fix_modify        3 temp mtem
            
thermo            1000
thermo_style      custom step c_mtem c_mwtem ke atoms v_nH2  v_nbH2   

run               200000
write_data        C39_H2_1020watergcmc0.7ns.data
run               300000
write_data        C39_H2_1020watergcmc1ns.data
run               300000
write_data        C39_H2_1020watergcmc1.3ns.data
run               300000
write_data        C39_H2_1020watergcmc1.6ns.data
run               400000
write_data        C39_H2_1020watergcmc2ns.data
